...
[delta_x, delta_y] = PositionDisturbance(0, 0, 0.05, DisturbanceTimesTh, 0, 1); 
for i = 1:DisturbanceTimesTh
    dCtrPtsPositions = array.dCtrPtsPositions;
    dCtrPtsPositions(:, 1:2) = dCtrPtsPositions(:, 1:2) + [delta_x(i), delta_y(i)]; 
...
ATF.irMeasured.HDPerturbationCoordinates = [delta_x; delta_y];
ATF.irMeasured.HD = HD;

% Bright
DisturbanceTimesTh = 50;
HB = zeros(size(bCtrPtsPositions, 1), size(s, 1), 1601, DisturbanceTimesTh);
[delta_x, delta_y] = PositionDisturbance(0, 0, 0.05, DisturbanceTimesTh, 0, 2); 
for i = 1:DisturbanceTimesTh
